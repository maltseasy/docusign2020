...
const declarationEmbeddedSigning = exports
    , eg = 'declaration' // This example reference.
    , mustAuthenticate = '/ds/mustAuthenticate'
    , login = '/ds/login'
    , minimumBufferMin = 3
    , signerClientId = 1000 // The id of the signer within this application.
    , demoDocsPath = path.resolve(__dirname, '../documents')
    , pdf1File = 'R.9_Declaration_of_Authenticity.pdf'
 ...
 */
declarationEmbeddedSigning.createController = async (req, res) => {
    ...
    try {
        results = await declarationEmbeddedSigning.worker (args)
    }
    catch (error) {
 ...
 */
// ***DS.snippet.0.start
declarationEmbeddedSigning.worker = async (args) => {
...
function makeEnvelope(args){
    ...
    doc1.name = 'Declaration of Authenticity'
    doc1.documentId = '3';
 ...
 */
declarationEmbeddedSigning.getController = (req, res) => {
    ...
    if (tokenOK) {
        res.render('pages/examples/declarationEmbeddedSigning', {
