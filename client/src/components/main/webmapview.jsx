      ...
      .then(
        ([
          ...
          const map = new ArcGISMap("map", {
            basemap: "topo-vector",
            center: [
              this.state.sites[0].fsc_longitude,
              this.state.sites[0].fsc_latitude,
            ],
            zoom: 5,
          });
          ...
          var covidTravelTemplate = new InfoTemplate(
            "${COUNTRY}",
            "<b>Travel Advisory: </b><p>${TravelAdvisory}</p><br><p>${RestrictionInfo}</p>"
          );

          var covidTravelLayer = new FeatureLayer(
            "https://services1.arcgis.com/vY6WuhLW0HkFe6Fl/arcgis/rest/services/TravelAdvisoriesV3/FeatureServer/0",
            {
          ...
          var covidCasesTemplate = new InfoTemplate(
            "${Country_Region}",
            "<b>Active Cases: </b><p>${Active}</p><br><b>Confirmed Cases: </b><p>${Confirmed}</p><br><b>Deaths: </b><p>${Deaths}</p><br><b>Recovered Cases: </b><p>${Recovered}</p><br>"
          );
          var covidCasesLayer = new FeatureLayer(
          ...
          var corruptionIndexTemplate = new InfoTemplate(
            "${NAME}",
            "<b>CPI Score: </b><p>${CPI_Score_}</p><br><b>Rank: </b><p>${Rank}</p><br><b>Upper CI: </b><p>${Upper_CI}</p><br><b>Lower CI: </b><p>${Lower_CI}</p><br>"
          );
          var corruptionIndexLayer = new FeatureLayer(
            "https://services1.arcgis.com/RTK5Unh1Z71JKIiR/arcgis/rest/services/Corruption_Perception_Index_countries_layer/FeatureServer/0",
            {
              outFields: ["*"],
              infoTemplate: corruptionIndexTemplate,
            }
          );
          var corruptionIndexRenderer = new SimpleRenderer(
          ...
          map.on("load", () => {
            var iconPath =
              "M32, 2a20, 20, 0, 0, 0-20, 20c0, 18, 20, 40, 20, 40s20-22, 20-40A20, 20, 0, 0, 0, 32, 2zm0, 28a8, 8, 0, 1, 1, 8-8, 8, 8, 0, 0, 1-8, 8z";
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} sm={3}>
            <div className="webmap-controls" style={{ paddingTop: 10 }}>
              <Accordion style={{ borderRadius: 5, marginBottom: 20 }}>
              ...
              <div id="layerListDom" style={{marginTop: 10, borderRadius: 10}}></div>
            </div>
            <div id="info">
              <div id="legend"></div>
            </div>
